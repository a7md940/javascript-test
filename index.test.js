...
describe('List', () => {
  ...
  describe('remove', () => {
    it('[1] should remove b from the first of the list a', () => {
      const list = List({ initial: [-1, 1, 2, 3, 4], initialOrder: true })
      const expected = [1, 2, 3, 4];
      const actual = list.remove(-1).items
      expect(actual).toEqual(expected)
    })
    it('[2] should remove b from the middle of the list a', () => {
      const list = List({
        initial: [
          { value: -1 },
          { value: 1 },
      ...
      const expected = [{ value: -1 }, { value: 1 }, { value: 3 }, { value: 4 }];
      const actual = list.remove({ value: 2 }).items
      expect(actual).toEqual(expected)
    })
    it('[3] should remove b from the last of list a', () => {
      const list = List({ initial: [1, 2, 3, 4], initialOrder: true })
  ...
  describe('findIndex', () => {
    it('should findIndex of b at list a (array of numbers)', () => {
      const list = List({ initial: [1, 2, 3, 4], initialOrder: true })
      const expected = 2;
    ...
    it('should findIndex of b at list a (array of objects)', () => {
  ...
  describe('insert', () => {
    ...
    it('(1) should insert b into the middle of the list a', () => {
